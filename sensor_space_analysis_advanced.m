...
for comp = 1:NCOMP 
    subplot(4,2,comp*2),mrC.Simulate.plotOnEgi(A_Vernier_all(:,comp)*-1);axis tight equal;caxis(caxisR2);
    ...
    subplot(4,2,comp*2-1),mrC.Simulate.plotOnEgi(A_Letter_all(:,comp));axis tight equal;caxis(caxisR2);
    title(['RC' num2str(comp)],'fontsize',FS,'Color',Cols(comp,:));%,'fontweight','normal');%colorbar;
    if comp==1
...
Ver_cmplx = -1*Ver_cos-(Ver_sin*1i);
...
print(FIG,['../Presentation/TopoMap_individuals/RCA_1f1_AverageAll'],'-r300','-dtiff');
close all;
